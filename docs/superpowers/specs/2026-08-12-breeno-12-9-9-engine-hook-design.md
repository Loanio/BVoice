...
兼容层继续调用用户提供的 GPT-SoVITS 服务根地址。不调用或代理小布内部云端接口，也不要求第三方服务修改协议。
...
2. 每次 `O0(String)` 只追加非空分片，并阻止原 `streamSpeak`。若宿主菜单路径未发送 `P0`，首个 `O0` 会创建 listener/bundle 为空的隐式代际。
3. `J0()` 冻结当前缓冲，将分片按原顺序拼接后提交一次第三方合成；隐式代际同样只提交一次。
...
- 实机 12.9.9：Vector 重载后出现 `engine=true`；普通播报与流式回答至少各命中一次 `intercepted`；观察到对用户配置服务的请求；第三方音频可听；停止、连续提问、服务离线回退和配置热更新通过。
